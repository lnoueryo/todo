...
export class UpdateTaskUsecase {
  ...
  > {
    try {
      ...
      const targetTasks = await this.taskRepository.getByIds(taskIds)
      ...
      const updateTasks = params.map((update) => {
        ...
        return this.taskRepository.updateTask(update.id, updateTask)
      })
      await Promise.all(updateTasks)
      const tasks = await this.taskRepository.getByUserId(user.id)
      return {
        success: {
          tasks,
        }
